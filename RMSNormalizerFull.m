directory = 'D:\MMusSamples\SeperatedTones';
savedir = 'D:\MMusSamples\FullNormalised';
cd(directory);
...
for DirNum = 1:size(subFolderNames,2)
    ...
    for i = 1:AudioFileMaxNumbers
        ...
        SusData = Data((2*samplerate):(3.5*samplerate));
        Data = Data((0.6*samplerate):length(Data));

        RootMeanSquare = rms(SusData);
        ...
        if (PassFile == 1)
            ...
            saveStr = strcat(savename, "-FullNormalised.wav");
